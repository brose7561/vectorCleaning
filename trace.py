...
def trace(src, dst, a):
    ...
    line  = (bgr.max(2) <= a.ink).astype(np.uint8)                 # black outline
    solid = (bgr.min(2) < a.paper).astype(np.uint8)                # everything not background
    body = np.logical_and(solid, ~line.astype(bool))
    ...
    masks = [(solid, hx(cols[0]) if len(cols) else '#808080', 'base')]   # base kills hairline seams
    masks += [((np.all(bgr == c, 2) & body).astype(np.uint8), hx(c), f'tone{i}')
              for i, c in enumerate(cols[1:], 1)]
    masks += [(line, '#000000', 'outline')]
    ...
    for txt, c, name in layers:                                    # potrace's <g> keeps its transform
        gs = '\n'.join(re.sub(r'\s+fill="[^"]*"', '', g) for g in re.findall(r'<g\b.*?</g>', txt, re.S))
        if gs.strip(): out.append(f'<g id="{name}" fill="{c}">\n{gs}\n</g>')
    out.append('</svg>')
    open(dst, 'w').write('\n'.join(out) + '\n')
...
p.add_argument('--backend', choices=['auto', 'potrace', 'vtracer'], default='auto')
